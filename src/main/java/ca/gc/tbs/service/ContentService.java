...
public class ContentService {
    ...
    private static final Pattern POSTAL_CODE_PATTERN = Pattern
            .compile("[A-Za-z]\\s*\\d\\s*[A-Za-z]\\s*[ -]?\\s*\\d\\s*[A-Za-z]\\s*\\d");
    private static final Pattern PASSPORT_PATTERN = Pattern.compile("\\b([A-Za-z]{2}\\s*\\d{6})\\b");
    private static final Pattern SIN_PATTERN = Pattern
            .compile("(\\d{3}\\s*\\d{3}\\s*\\d{3}|\\d{3}\\D*\\d{3}\\D*\\d{3})");
    private static final Pattern PHONE_PATTERN_1 = Pattern
            .compile("(\\+\\d{1,2}\\s?)?1?\\-?\\.?\\s?\\(?\\d{3}\\)?[\\s.-]?\\d{3}[\\s.-]?\\d{4}");
    private static final Pattern PHONE_PATTERN_2 = Pattern.compile(
            "(?:(?:\\+?1\\s*(?:[.-]\\s*)?)?(?:\\(\\s*([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9])\\s*\\)|([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9]))\\s*(?:[.-]\\s*)?)?([2-9]1[02-9]|[2-9][02-9]1|[2-9][02-9]{2})\\s*(?:[.-]\\s*)?([0-9]{4})(?:\\s*(?:#|x\\.?|ext\\.?|extension)\\s*(\\d+))?");
    private static final Pattern EMAIL_PATTERN = Pattern
            .compile("([a-zA-Z0-9_\\-\\.]+)\\s*@([\\sa-zA-Z0-9_\\-\\.]+)[\\.\\,]([a-zA-Z]{1,5})");
    ...
    /**
     * Cleans the names of persons in the provided content by replacing them with
     * '#' characters.
     * Uses StanfordCoreNLP for natural language processing and entity recognition.
     * Reverses the list of entity mentions to replace them from the end of the
     * string,
